...
import net.thirdlife.iterrpg.procedures.FrostspikeHitProcedure;
import net.thirdlife.iterrpg.procedures.FrostspikeHitBlockProcedure;
import net.thirdlife.iterrpg.procedures.FrostSpikeTickProcedure;
...
public class FrostSpikeEntity extends AbstractArrow implements ItemSupplier {
	...
	public FrostSpikeEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(IterRpgModEntities.FROST_SPIKE.get(), world);
	}
	...
	public void onHitEntity(EntityHitResult entityHitResult) {
		...
		FrostspikeHitProcedure.execute(this.level(), entityHitResult.getEntity(), this, this.getOwner());
	}

	@Override
	public void onHitBlock(BlockHitResult blockHitResult) {
		super.onHitBlock(blockHitResult);
		FrostspikeHitBlockProcedure.execute(this.level(), blockHitResult.getBlockPos().getX(), blockHitResult.getBlockPos().getY(), blockHitResult.getBlockPos().getZ(), this);
	}

	@Override
	public void tick() {
		super.tick();
		FrostSpikeTickProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
		if (this.inGround)
			this.discard();
	}

	public static FrostSpikeEntity shoot(Level world, LivingEntity entity, RandomSource source) {
	...
	public static FrostSpikeEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		FrostSpikeEntity entityarrow = new FrostSpikeEntity(IterRpgModEntities.FROST_SPIKE.get(), entity, world);
	...
	public static FrostSpikeEntity shoot(LivingEntity entity, LivingEntity target) {
		FrostSpikeEntity entityarrow = new FrostSpikeEntity(IterRpgModEntities.FROST_SPIKE.get(), entity, entity.level());
