...
public class AttractCastProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (world instanceof Level _level) {
			if (!_level.isClientSide()) {
				_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.end_portal_frame.fill")), SoundSource.PLAYERS, (float) 0.5, (float) 0.7);
			} else {
				_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.end_portal_frame.fill")), SoundSource.PLAYERS, (float) 0.5, (float) 0.7, false);
			}
		}
		push = power * (-0.5);
		hit = true;
		for (int index0 = 0; index0 < (int) (4 * power * 8); index0++) {
			if (hit) {
				if (world.getBlockState(BlockPos.containing(x + xdir * dist, yheight + ydir * dist, z + zdir * dist)).canOcclude()) {
					hit = false;
				}
				world.addParticle((SimpleParticleType) (IterRpgModParticleTypes.ELEMENTAL_VOID.get()), (x + xdir * dist + dist * Mth.nextDouble(RandomSource.create(), -0.25, 0.25)),
						(yheight + ydir * dist + dist * Mth.nextDouble(RandomSource.create(), -0.25, 0.25)), (z + zdir * dist + dist * Mth.nextDouble(RandomSource.create(), -0.25, 0.25)), (xdir * (-0.25)), (ydir * (-0.25)), (zdir * (-0.25)));
				{
