...
public class VoidbeamCastProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		hitcontinue = true;
		for (int index0 = 0; index0 < (int) (50 + Math.round(25 * power)); index0++) {
			if (hitcontinue) {
				xnew = xnew + xdir * dist;
				ynew = ynew + ydir * dist;
				znew = znew + zdir * dist;
				if (world instanceof ServerLevel _level)
					_level.sendParticles((SimpleParticleType) (IterRpgModParticleTypes.ELEMENTAL_VOID.get()), xnew, ynew, znew, 1, 0, 0, 0, 0);
				{
