...
public class FireSealBrokenProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		if (!IterRpgModVariables.MapVariables.get(world).unlocked_fire) {
			...
			for (int index1 = 0; index1 < 200; index1++) {
				world.addParticle((SimpleParticleType) (IterRpgModParticleTypes.FIRE_SIGIL.get()), (x + Mth.nextDouble(RandomSource.create(), -20, 20)), (y + Mth.nextDouble(RandomSource.create(), -2, 6)),
						(z + Mth.nextDouble(RandomSource.create(), -20, 20)), 0, 0.05, 0);
			}
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = IterRpgModEntities.FIRE_ELEMENTAL.get().spawn(_level, BlockPos.containing(x + 0.5, y, z + 0.5), MobSpawnType.MOB_SUMMONED);
			...
			if (!world.isClientSide() && world.getServer() != null)
				world.getServer().getPlayerList().broadcastSystemMessage(Component.literal((Component.translatable("iterpg.global.fireseal").getString())), false);
			if (!world.isClientSide() && world.getServer() != null)
				world.getServer().getPlayerList().broadcastSystemMessage(Component.literal((Component.translatable("iterpg.global.fireseal1").getString())), false);
		}
	}
}
